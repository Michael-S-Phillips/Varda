...
class ROISelector(pg.GraphicsObject):
    ...
    def __init__(self, color=None, mode=ROIMode.FREEHAND):
        ...
        # Basic properties
        self.mode = mode
        self.color = color if color else (0, 0, 255, 100)  # default: semi-transparent blue
        ...
        self.hoverPen = pg.mkPen(color=(255, 255, 0), width=self.penWidth+1)  # Yellow highlight
        ...
        self.instructionItem = pg.TextItem(
            text=self.instructions[self.mode], 
            color=(255, 255, 255),
            anchor=(0, 0)
        )
    ...
    def eventFilter(self, obj, ev):
        ...
        if ev.type() == ev.Type.KeyPress:
            if ev.key() == Qt.Key.Key_Escape:
                ...
                if self.mode == ROIMode.POLYGON and self.pts is not None and len(self.pts[0]) >= 3:
                    self.completeDrawing()
                    return True
            elif ev.key() == Qt.Key.Key_Backspace:
                # Remove the last point in polygon mode
                if self.mode == ROIMode.POLYGON and self.pts is not None and len(self.pts[0]) > 0:
                    self.pts[0].pop()
                    self.pts[1].pop()
                    self.updatePath()
                    return True
        ...
        # Mouse move events
            ...
            if self.isDrawing:
                ...
                elif (self.mode == ROIMode.RECTANGLE or self.mode == ROIMode.ELLIPSE) and self.startPoint is not None:
        ...
        # Mouse release events
            ...
            if ev.button() == Qt.MouseButton.LeftButton:
                if self.mode == ROIMode.FREEHAND:
                    ...
                    if self.rect is not None and self.rect.width() > 5 and self.rect.height() > 5:
        ...
        if self.path is not None:
            ...
            if self.mode == ROIMode.POLYGON and self.isDrawing:
                ...
                if self.tempPoint is not None and len(self.pts[0]) > 0:
                    ...
                    p.drawLine(
                        QPointF(self.pts[0][-1], self.pts[1][-1]), 
                        self.tempPoint
                    )
