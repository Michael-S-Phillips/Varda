...
from core.entities import Metadata, Band
...
class MetadataEditor(QDialog):
    ...
    def setupBandsTab(self):
        ...
        self.bandTable.setHorizontalHeaderLabels(["Name", "Red Channel", "Green Channel", "Blue Channel"])
    ...
    def generateWavelengths(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Input Error", f"Invalid input for wavelength generation: {e}")
                ...
                for i, band in enumerate(bands):
                    if i < len(image.band):
                        self.proj.updateBand(
                            self.imageIndex, i, 
                            name=band.name, r=band.r, g=band.g, b=band.b
                        )
                    else:
