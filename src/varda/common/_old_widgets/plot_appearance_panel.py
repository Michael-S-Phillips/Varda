...
class PlotAppearancePanel(QWidget):
    ...
    def _init_ui(self):
        ...
        self.x_label_combo.addItems(
            ["Wavelength (nm)", "Band Number", "Frequency (Hz)", "Energy (eV)"]
        )
        ...
        self.y_label_combo.addItems(
            ["Intensity", "Reflectance", "Radiance", "Counts", "Normalized"]
        )
        ...
        self.title_combo.addItems(
            ["Spectral Plot", "Pixel Spectrum", "ROI Spectrum", "Custom"]
        )
    ...
    def _apply_property_to_plot(self, property_name: str, value):
        ...
        try:
            if property_name == "background_color":
                ...
                self.plot_widget.plot_widget.showGrid(
                    x=value, y=self.grid_y_checkbox.isChecked()
                )

            elif property_name == "grid_y":
                self.plot_widget.plot_widget.showGrid(
                    x=self.grid_x_checkbox.isChecked(), y=value
                )
