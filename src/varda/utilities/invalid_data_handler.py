...
class InvalidDataHandler:
    ...
    def detect_invalid_values(data: np.ndarray) -> Tuple[np.ndarray, Dict[str, int]]:
        ...
        try:
            ...
            stats = {
                ...
                "valid_percentage": (
                    (np.sum(finite_mask) / len(data)) * 100 if len(data) > 0 else 0
                ),
            }
        ...
        try:
            ...
            if stats["valid_count"] == 0:
                ...
                return (
                    fallback_data,
                    False,
                    "All values were invalid, replaced with zeros",
                )
            ...
            if preserve_shape and data.ndim > 1:
                try:
                    ...
                    logger.warning(
                        "Could not restore original shape, returning flattened data"
                    )
    ...
    def handle_spectral_pair(
        ...
        try:
            ...
            wl_invalid_mask, wl_stats = InvalidDataHandler.detect_invalid_values(
                wavelengths
            )
            val_invalid_mask, val_stats = InvalidDataHandler.detect_invalid_values(
                values
            )
            ...
            if sync_removal and strategy == InvalidValueStrategy.REMOVE_INVALID:
                ...
                clean_wavelengths, wl_success, wl_message = (
                    InvalidDataHandler.handle_invalid_values(wavelengths, strategy)
                )
                clean_values, val_success, val_message = (
                    InvalidDataHandler.handle_invalid_values(values, strategy)
                )
    ...
    def validate_spectral_data_quality(
        wavelengths: np.ndarray, values: np.ndarray, min_valid_percentage: float = 50.0
        ...
        try:
            ...
            quality_report = {
                ...
                "overall_valid_percentage": min(
                    wl_stats["valid_percentage"], val_stats["valid_percentage"]
                ),
            ...
            if wl_stats["valid_count"] > 1:
                ...
                quality_report["has_monotonic_wavelengths"] = np.all(
                    np.diff(valid_wl) > 0
                ) or np.all(np.diff(valid_wl) < 0)
            ...
            if not quality_report["length_match"]:
                quality_report["quality_issues"].append(
                    "Wavelength and value array length mismatch"
                )
            ...
            if not quality_report["has_reasonable_range"]:
                quality_report["quality_issues"].append(
                    "Spectral values have no variance"
                )

            is_good_quality = (
                quality_report["overall_valid_percentage"] >= min_valid_percentage
                and quality_report["length_match"]
                and len(quality_report["quality_issues"]) == 0
            )
