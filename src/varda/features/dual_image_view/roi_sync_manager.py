...
class ROISyncManager(QObject):
    ...
    roi_sync_failed = pyqtSignal(str, int, int, str)  # roi_id, source_index, target_index, error
    ...
    def setup_roi_sync(self, primary_index: int, secondary_index: int):
        ...
        logger.debug(f"ROI sync set up for images {primary_index} <-> {secondary_index}")
    ...
    def cleanup_roi_sync(self, primary_index: int, secondary_index: int):
        ...
        logger.debug(f"ROI sync cleaned up for images {primary_index} <-> {secondary_index}")
    ...
    def sync_roi(self, roi_id: str, source_index: int, target_index: int) -> bool:
        ...
        try:
            ...
            transformed_roi = self._transform_roi_coordinates(source_roi, source_index, target_index)
            ...
            if synced_roi_id:
                ...
                error_msg = f"Failed to create synced ROI in target image {target_index}"
    ...
    def remove_synced_roi(self, roi_id: str, source_index: int, target_index: int) -> bool:
        """
        ...
        if pair_key in self._roi_mappings and roi_id in self._roi_mappings[pair_key]:
            ...
            try:
                ...
                if hasattr(self.proj, 'roi_manager'):
                    success = self.proj.roi_manager.remove_roi(target_index, synced_roi_id)
                else:
                ...
                if success:
                    ...
                    logger.debug(f"Removed synced ROI {synced_roi_id} from image {target_index}")
                    return True
    ...
    def get_synced_roi_id(self, source_roi_id: str, source_index: int, target_index: int) -> Optional[str]:
        ...
        try:
            if isinstance(roi, FreehandROI):
                ...
                if hasattr(roi, 'points') and roi.points is not None:
                    new_roi.points = roi.points.copy() if hasattr(roi.points, 'copy') else roi.points
    ...
    def _create_or_update_synced_roi(self, roi, source_index: int, target_index: int, source_roi_id: str) -> Optional[str]:
        ...
        try:
            ...
            if pair_key in self._roi_mappings and source_roi_id in self._roi_mappings[pair_key]:
                existing_synced_id = self._roi_mappings[pair_key][source_roi_id]
            
            if existing_synced_id:
                # Update existing synced ROI
                return self._update_existing_synced_roi(existing_synced_id, roi, target_index)
            else:
    ...
    def _update_existing_synced_roi(self, synced_roi_id: str, roi, target_index: int) -> Optional[str]:
        ...
        try:
            ...
            for pair_key, mapping in self._roi_mappings.items():
                if image_index in pair_key:
                    other_index = pair_key[1] if pair_key[0] == image_index else pair_key[0]
