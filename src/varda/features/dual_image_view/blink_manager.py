...
class BlinkManager(QObject):
    ...
    # Signals
    blink_state_changed = pyqtSignal(
        bool
    )  # True = primary visible, False = secondary visible
    ...
    def set_fade_effect(self, enabled: bool, duration_ms: int = 100):
        ...
        logger.debug(
            f"Fade effect {'enabled' if enabled else 'disabled'} with duration {self._fade_duration}ms"
        )
    ...
    def _apply_fade_effect(self, primary_visible: bool):
        ...
        logger.debug(
            f"Fade effect applied (placeholder): primary_visible={primary_visible}"
        )
