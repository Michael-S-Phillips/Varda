...
class DualImageLinkManager(QObject):
    ...
    def create_link(self, primary_index: int, secondary_index: int, 
        ...
        if self.are_images_linked(primary_index, secondary_index):
            logger.warning(f"Images {primary_index} and {secondary_index} are already linked")
            return False
        ...
        if not self._validate_link_compatibility(primary_index, secondary_index, config.link_type):
            return False
        ...
        try:
            ...
            logger.info(f"Successfully linked images {primary_index} and {secondary_index}")
            self.images_linked.emit(primary_index, secondary_index)
            return True
            
        except Exception as e:
            logger.error(f"Failed to create link between images {primary_index} and {secondary_index}: {e}")
            return False
    ...
    def remove_link(self, primary_index: int, secondary_index: int) -> bool:
        ...
        if pair_key not in self._image_pairs:
            logger.warning(f"No link exists between images {primary_index} and {secondary_index}")
            return False
        ...
        logger.info(f"Successfully unlinked images {primary_index} and {secondary_index}")
    ...
    def update_link_config(self, primary_index: int, secondary_index: int, 
                          config: DualImageConfig) -> bool:
    ...
    def transform_coordinates(self, from_index: int, to_index: int, 
        ...
        if pair.config.link_type == LinkType.PIXEL_BASED:
            ...
            if pair.transform_matrix is not None:
                ...
                logger.warning("Geospatial coordinate transformation not yet implemented")
                return (x, y)
    ...
    def cleanup_invalid_pairs(self):
        ...
        for pair_key, pair in self._image_pairs.items():
            if (pair.primary_index not in valid_image_indices or 
                pair.secondary_index not in valid_image_indices):
    ...
    def _validate_link_compatibility(self, index1: int, index2: int, link_type: LinkType) -> bool:
        ...
        try:
            ...
            if link_type == LinkType.PIXEL_BASED:
                ...
                if image1.raster.shape[:2] != image2.raster.shape[:2]:
                    logger.warning(f"Images {index1} and {index2} have different dimensions for pixel-based linking")
                ...
                if not (hasattr(image1.metadata, 'crs') and hasattr(image2.metadata, 'crs')):
                    logger.warning(f"Images {index1} and {index2} lack geospatial metadata")
                    return True  # Allow anyway, user may know what they're doing
