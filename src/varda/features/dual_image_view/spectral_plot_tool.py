...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QCheckBox, QSpinBox, QGroupBox, QButtonGroup, QRadioButton
)
...
class SpectralPlotToolPanel(DualImageToolPanel):
    ...
    def _setup_controls(self):
        ...
        self.auto_label_cb.setToolTip("Automatically label spectra with their image coordinates")
...
class SpectralPlotTool(DualImageToolBase):
    ...
    def _on_activate(self) -> bool:
        ...
        self.status_changed.emit(self.tool_name, "Ready - Click on images to plot spectra")
        return True
    ...
    def _on_click(self, image_index: int, x: int, y: int, view_type: str) -> bool:
        ...
        try:
            ...
            wavelengths, wavelength_type = WavelengthProcessor.process_wavelength_data(
                source_image.metadata.wavelengths, 
                source_image.raster.shape[2]
            )
            
            # Get spectrum data
            spectrum = BoundsValidator.safe_pixel_access(source_image.raster, safe_x, safe_y)
            ...
            if len(clean_spectrum) == 0:
                self.status_changed.emit(self.tool_name, f"No valid spectral data at ({x}, {y})")
                return False
            
            # Generate spectrum label
            if self._auto_label:
                source_name = "Primary" if source_index == self._primary_index else "Secondary"
            ...
            color_index = len(self._plot_widget.spectra) % len(self._plot_widget.default_colors)
            ...
            if spectrum_id:
                ...
                # Update status
                click_source = "Primary" if image_index == self._primary_index else "Secondary"
                data_source = "Primary" if source_index == self._primary_index else "Secondary"
                status = f"Plotted spectrum from {data_source} image (clicked on {click_source})"
                self.status_changed.emit(self.tool_name, status)
                
                logger.info(f"Added spectrum: {label} from image {source_index} at ({safe_x}, {safe_y})")
                return True
            else:
                self.status_changed.emit(self.tool_name, f"Failed to extract spectrum at ({x}, {y})")
                return False
    ...
    def _on_images_changed(self, primary_index: int, secondary_index: int):
        ...
        logger.debug(f"SpectralPlotTool updated for images: {primary_index}, {secondary_index}")
    ...
    def get_tool_info(self) -> Dict[str, Any]:
        ...
        base_info.update({
            ...
            "spectrum_count": len(self._plot_widget.get_all_spectra_info()) if self._plot_widget else 0,
        })
        return base_info