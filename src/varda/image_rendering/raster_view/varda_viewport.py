...
class VardaRasterLayer(pg.GraphicsObject):
    def __init__(self, raster: VardaRaster):
        self.imageRenderer = ImageRenderer(raster)
        self.imageItem = VardaImageItem(self.imageRenderer)
        self.item


class VardaViewport(QWidget):
    """
    Viewport for displaying rasters, vectors, ROIs etc.
    """
    ...
    def setMouseEnabled(self, x: bool, y: bool):
        """Set whether mouse interaction is enabled for panning/zooming in the viewport."""
        self._vb.setMouseEnabled(x=x, y=y)

    def getLayers(self):
        return self.layers

    def addLayer(self, layer):
        if isinstance(layer, ImageRenderer):
            self.layers.append(VardaImageItem(layer))
            self._vb.addItem(self.layers[-1])

            layer.sigShouldRefresh.connect(self.autoRefresh)
