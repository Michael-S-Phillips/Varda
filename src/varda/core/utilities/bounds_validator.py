...
class BoundsValidator:
    ...
    def validate_roi_bounds(
        ...
        if (x >= 0 and y >= 0 and 
            x + width <= img_width and y + height <= img_height and
        ...
        if (clipped_x != x or clipped_y != y or 
            clipped_width != width or clipped_height != height):
            logger.info(
    ...
    def safe_roi_access(
        ...
        """
        try:
            is_valid, (safe_x, safe_y, safe_width, safe_height) = BoundsValidator.validate_roi_bounds(
            ...
            roi_data = raster[safe_y:safe_y + safe_height, safe_x:safe_x + safe_width, :]
            
            # Validate the returned data
            if np.any(np.isnan(roi_data)) or np.any(np.isinf(roi_data)):
                logger.warning(f"Invalid data detected in ROI ({safe_x}, {safe_y}, {safe_width}, {safe_height})")
                roi_data = np.nan_to_num(roi_data, nan=0.0, posinf=0.0, neginf=0.0)
            
            return roi_data
            
        except Exception as e:
            logger.error(f"Error accessing ROI data at ({x}, {y}, {width}, {height}): {e}")
            return np.zeros((1, 1, raster.shape[2]), dtype=raster.dtype)