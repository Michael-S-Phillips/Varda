...
class WavelengthProcessor:
    ...
    def process_wavelength_data(
        wavelengths: Union[np.ndarray, list, None], band_count: int
        ...
        try:
            ...
            try:
                ...
                if wavelengths.dtype.kind in [
                    "U",
                    "S",
                    "O",
                ]:  # Unicode, byte string, or object
                    # Strip whitespace and attempt conversion
                    numeric_wavelengths = np.char.strip(wavelengths.astype(str)).astype(
                        float
                    )

                    # Validate converted values
                    if np.any(np.isnan(numeric_wavelengths)) or np.any(
                        np.isinf(numeric_wavelengths)
                    ):
