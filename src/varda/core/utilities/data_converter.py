...
class DataConverter:
    ...
    def safe_array_conversion(
        ...
        try:
            ...
            if data is None:
                ...
                return DataConverter._create_fallback_array(
            ...
            try:
                ...
                error_message = f"Failed to convert data to {target_dtype.__name__}: {e}"
                logger.warning(error_message)
            ...
            if array_data.ndim > 1:
                ...
                logger.info(f"Flattened array from shape {original_shape} to {array_data.shape}")
            ...
            if expected_length is not None and len(array_data) != expected_length:
                ...
                array_data = DataConverter._resize_array(array_data, expected_length, target_dtype)
            ...
            if target_dtype == float:
                ...
                if np.any(invalid_mask):
                    ...
                    logger.warning(f"Found {invalid_count} invalid values, replacing with interpolation")
                    array_data = DataConverter._fix_invalid_values(array_data, invalid_mask)
            ...
            logger.debug(f"Successfully converted data to {target_dtype.__name__} array of length {len(array_data)}")
    ...
    def _attempt_alternative_conversion(
        ...
        try:
            ...
            if hasattr(data, '__iter__') and not isinstance(data, str):
                ...
                if converted_items:
                    logger.info("Successfully converted via element-by-element conversion")
    ...
    def _resize_array(
        ...
        if current_length > target_length:
            ...
            if current_length > 0:
                ...
                logger.info(f"Padding array from {current_length} to {target_length} with value {pad_value}")
            else:
                # Pad with zeros
    ...
    @staticmethod
    def _fix_invalid_values(array_data: np.ndarray, invalid_mask: np.ndarray) -> np.ndarray:
        ...
        if len(valid_indices) == 0:
            ...
            logger.info(f"Replaced {len(invalid_indices)} invalid values with {valid_value}")
        else:
    ...
    def validate_spectral_data_compatibility(
        ...
        try:
            ...
            if min_length > 0:
                ...
                return True, f"Arrays truncated to length {min_length}", (corrected_wavelengths, corrected_values)
            else:
