...
class ROIDrawingManager(QObject):
    ...
    def createToolbar(self):
        ...
        self.action_freehand.triggered.connect(lambda: self.setDrawingMode(ROIMode.FREEHAND))
        
        self.action_rectangle = QAction("Rectangle", self.toolbar)
        self.action_rectangle.setCheckable(True)
        self.action_rectangle.triggered.connect(lambda: self.setDrawingMode(ROIMode.RECTANGLE))
        
        self.action_ellipse = QAction("Ellipse", self.toolbar)
        self.action_ellipse.setCheckable(True)
        self.action_ellipse.triggered.connect(lambda: self.setDrawingMode(ROIMode.ELLIPSE))
        
        self.action_polygon = QAction("Polygon", self.toolbar)
        self.action_polygon.setCheckable(True)
        self.action_polygon.triggered.connect(lambda: self.setDrawingMode(ROIMode.POLYGON))
    ...
    def onDrawingComplete(self, roi_data):
        ...
        try:
            ...
            if image and hasattr(image, 'raster'):
                ...
                mean_spectrum = np.nanmean(array_slice, axis=0) if array_slice.size > 0 else None
    ...
    def displayROI(self, roi):
        ...
        try:
            ...
            # Set the points
            selector.pts = [roi.points[0].tolist(), roi.points[1].tolist()] \
                if isinstance(roi.points, np.ndarray) else roi.points
            selector.updatePath()
            ...
            if hasattr(self.raster_view, 'contextView') and hasattr(self.raster_view, 'contextImage'):
                context_selector = ROISelector(roi.color)
                context_selector.pts = selector.pts.copy() if hasattr(selector, 'pts') and selector.pts is not None else None
    ...
    def setupViewChangeHandlers(self):
        ...
        if hasattr(self.raster_view, 'mainView') and self.raster_view.mainView is not None:
            # Check if the view has the signal before connecting
            if hasattr(self.raster_view.mainView, 'sigTransformChanged'):
                self.raster_view.mainView.sigTransformChanged.connect(self.onViewTransformChanged)
            else:
                logger.warning("MainView does not have sigTransformChanged signal")
        
        if hasattr(self.raster_view, 'contextView') and self.raster_view.contextView is not None:
            # Check if the view has the signal before connecting
            if hasattr(self.raster_view.contextView, 'sigTransformChanged'):
                self.raster_view.contextView.sigTransformChanged.connect(self.onViewTransformChanged)
            else:
    ...
    def getRoiById(self, roi_id):
        ...
        if hasattr(self.view_model, 'getRoi'):
            ...
            try:
                ...
                try:
                    image_index = getattr(self.view_model, 'index', getattr(self.view_model, 'imageIndex', 0))
    ...
    def highlightROI(self, roi_id):
        ...
        if roi_id in self.roi_lookup:
            ...
            if roi and hasattr(roi, 'color'):
                ...
                r, g, b, a = roi.color
                highlight_color = (min(r + 80, 255), min(g + 80, 255), min(b + 80, 255), min(a + 50, 255))
    ...
    def getROIData(self, roi_id):
        ...
        if not hasattr(roi, 'array_slice') or roi.array_slice is None:
            try:
                # Extract image data for the ROI
                image = self.view_model.proj.getImage(self.view_model.imageIndex)
                if image and hasattr(image, 'raster') and hasattr(roi, 'points') and roi.points is not None:
                    # Create a mask from the ROI points
                    from skimage.draw import polygon
                    mask = np.zeros((image.raster.shape[0], image.raster.shape[1]), dtype=bool)
                    ...
                    roi.mean_spectrum = np.nanmean(roi.array_slice, axis=0) if roi.array_slice.size > 0 else None
                    
                    # Update the ROI in the project context
                    try:
                        if hasattr(self.view_model, 'updateRoi'):
                            self.view_model.updateRoi(roi_id, array_slice=roi.array_slice, mean_spectrum=roi.mean_spectrum)
                        elif hasattr(self.view_model, 'proj'):
                            self.view_model.proj.update_roi(roi_id, array_slice=roi.array_slice, mean_spectrum=roi.mean_spectrum)
                    except Exception as e:
