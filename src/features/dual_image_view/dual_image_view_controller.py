...
class DualImageViewController(QObject):
    ...
    def __init__(self, project_context: ProjectContext, parent=None):
        ...
        self._active_pair: Optional[Tuple[int, int]] = None  # (primary_index, secondary_index)
    ...
    def activate_dual_view(self, primary_index: int, secondary_index: int,
                          config: Optional[DualImageConfig] = None) -> bool:
        """
    ...
    def deactivate_dual_view(self) -> bool:
        ...
        if self._active_pair:
            self.roi_sync_manager.cleanup_roi_sync(self._active_pair[0], self._active_pair[1])
    ...
    def set_blink_interval(self, interval_ms: int) -> bool:
        ...
        success = self.blink_manager.set_blink_interval(self._current_config.blink_interval)
    ...
    def set_view_references(self, primary_view: QWidget, secondary_view: QWidget):
        ...
        logger.debug(f"View references set: primary={primary_view is not None}, secondary={secondary_view is not None}")
        ...
        if self._current_config:
            ...
            if mode == DualImageMode.OVERLAY:
                ...
                self.overlay_manager.set_overlay_opacity(self._current_config.overlay_opacity)
    ...
    def sync_navigation(self, source_index: int, view_state: Dict[str, Any]):
        ...
        """
        if (self._sync_in_progress or not self._is_dual_mode_active or 
            not self._active_pair or not self._current_config.sync_navigation):
            return
    ...
    def sync_roi(self, source_index: int, roi_id: str):
        ...
        if (not self._is_dual_mode_active or not self._active_pair or 
            not self._current_config.sync_rois):
            return
        ...
        if success:
            logger.debug(f"Successfully synced ROI {roi_id} from {source_index} to {target_index}")
        else:
    ...
    def _connect_signals(self):
        ...
        self.link_manager.navigation_sync_requested.connect(self._handle_navigation_sync)
    ...
    def _connect_blink_signals(self):
        ...
        self.blink_manager.blink_started.connect(lambda: self.blink_state_changed.emit(True))
        self.blink_manager.blink_stopped.connect(lambda: self.blink_state_changed.emit(False))

    def _setup_dual_mode(self):
        """Setup the dual view based on current configuration"""
        if not self._current_config or not self._primary_view or not self._secondary_view:
            logger.warning(f"Cannot setup dual mode - config={self._current_config is not None}, primary={self._primary_view is not None}, secondary={self._secondary_view is not None}")
            return
    ...
    def _setup_overlay_mode(self):
        ...
        if self._primary_view and self._secondary_view:
            success = self.overlay_manager.setup_overlay(self._primary_view, self._secondary_view)
            if success:
    ...
    def _setup_blink_mode(self):
        ...
        if self._primary_view and self._secondary_view:
            success = self.blink_manager.setup_blink(self._primary_view, self._secondary_view)
            if success:
                # Apply current settings
                self.blink_manager.set_blink_interval(self._current_config.blink_interval)
            ...
            logger.error(f"Cannot setup blink - views missing: primary={self._primary_view is not None}, secondary={self._secondary_view is not None}")
