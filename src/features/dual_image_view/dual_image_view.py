...
class DualImageView(QWidget):
    ...
    # Signals
    primary_image_changed = pyqtSignal(int)  # Emitted when primary image selection changes
    ...
    def set_primary_image(self, image_index: int) -> bool:
        ...
        self._update_view_container(self.primary_view_container, raster_view, 
    ...
    def set_secondary_image(self, image_index: int) -> bool:
        ...
        # Update container to actually show the raster view
        self._update_view_container(self.secondary_view_container, raster_view,
    ...
    def _connect_signals(self):
        ...
        self.controller.overlay_opacity_changed.connect(self._on_controller_opacity_changed)
    ...
    def _toggle_link(self):
        ...
        if self._is_linked:
            ...
            # Link
            logger.debug(f"Linking images: {self._primary_index} and {self._secondary_index}")
    ...
    def _can_link(self) -> bool:
        ...
        return (self._primary_index is not None and 
                self._secondary_index is not None and
    ...
    def _on_dual_view_activated(self, primary_index, secondary_index):
        ...
        if primary_view and secondary_view:
            ...
            logger.error(f"Views missing during activation: primary={primary_view is not None}, secondary={secondary_view is not None}")
    ...
    def _setup_navigation_sync(self):
        ...
        if not primary_view or not secondary_view:
            logger.error(f"Cannot setup navigation sync - missing views: primary={primary_view is not None}, secondary={secondary_view is not None}")
            return
        
        logger.debug(f"Setting up navigation sync between views for images {self._primary_index} and {self._secondary_index}")
    ...
    def _on_navigation_changed(self, source_index: int, view_state: dict):
        ...
        if self._is_linked and self.controller:
            logger.debug(f"Navigation changed in image {source_index}, syncing to other view")
