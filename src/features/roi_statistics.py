...
class ROIStatistics:
    ...
    def compare_bands(self, band1_idx: int, band2_idx: int) -> Dict[str, Any]:
        ...
        """
        if band1_idx < 0 or band1_idx >= self.n_bands or band2_idx < 0 or band2_idx >= self.n_bands:
            raise ValueError(f"Band indices out of range (0-{self.n_bands-1})")
            
        # Get valid data for both bands
        valid_mask = ~np.isnan(self.data[:, band1_idx]) & ~np.isnan(self.data[:, band2_idx])
...
def compare_rois(roi_stats1, roi_stats2):
    ...
    mean_diff_percent = mean_diff / np.where(roi_stats2.mean == 0, np.nan, roi_stats2.mean) * 100
