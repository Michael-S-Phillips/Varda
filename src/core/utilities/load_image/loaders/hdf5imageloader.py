        ...
        try:
            with h5py.File(filePath, "r") as hdf:
                ...
                if dataset is None:
                    ...
                    while queue and dataset is None:
                        ...
                        if isinstance(current, h5py.Dataset) and len(current.shape) >= 2:
                            if len(current.shape) == 3 or (len(current.shape) == 2 and current.shape[0] > 1 and current.shape[1] > 1):
                                dataset = current
                ...
                if len(data.shape) == 2:
                    ...
                    data = data[0] if data.shape[0] == 1 else data.reshape(-1, data.shape[-2], data.shape[-1])
            ...
            with h5py.File(filePath, "r") as hdf:
                ...
                if wavelengths is not None:
                    ...
                    if len(wavelengths) == raster.shape[2]:
                        ...
                        logger.warning("Wavelength count doesn't match band count, using band indices")
