...
class StretchAlgorithm:
    ...
    @staticmethod
    def compute_stretch(image_data: np.ndarray, **kwargs) -> Tuple[float, float, float, float, float, float]:
...
class SquareRootStretch(StretchAlgorithm):
    ...
    def compute_stretch(image_data: np.ndarray, **kwargs) -> Tuple[float, float, float, float, float, float]:
        ...
        if image_data.ndim != 3 or image_data.shape[2] < 3:
            ...
            return min_val, min_val + transformed_max**2, min_val, min_val + transformed_max**2, min_val, min_val + transformed_max**2
        ...
        return minR, minR + transformed_maxR**2, minG, minG + transformed_maxG**2, minB, minB + transformed_maxB**2
...
class LogarithmicStretch(StretchAlgorithm):
    ...
    def compute_stretch(image_data: np.ndarray, **kwargs) -> Tuple[float, float, float, float, float, float]:
        ...
        if image_data.ndim != 3 or image_data.shape[2] < 3:
            ...
            return min_val, min_val + np.exp(log_max) - 1, min_val, min_val + np.exp(log_max) - 1, min_val, min_val + np.exp(log_max) - 1
        ...
        return minR, minR + np.exp(log_maxR) - 1, minG, minG + np.exp(log_maxG) - 1, minB, minB + np.exp(log_maxB) - 1


class DecorrelationStretch(StretchAlgorithm):
    """Decorrelation stretch that enhances color differences."""
    ...
    def compute_stretch(image_data: np.ndarray, **kwargs) -> Tuple[float, float, float, float, float, float]:
        ...
        if image_data.ndim != 3 or image_data.shape[2] < 3:
            # Cannot do decorrelation on non-RGB data
        ...
        try:
            ...
            # For decorrelation, we need to know the statistical properties
            # Return the original min-max, but store the PCA info for later transformation
            minR, maxR = np.min(image_data[:,:,0]), np.max(image_data[:,:,0])
            minG, maxG = np.min(image_data[:,:,1]), np.max(image_data[:,:,1])
            minB, maxB = np.min(image_data[:,:,2]), np.max(image_data[:,:,2])
            
            # Store decorrelation parameters in kwargs for downstream processing
            # These will be used by the actual transformation functions
            kwargs["decorr_mean"] = mean_vec
            kwargs["decorr_eigenvecs"] = eigenvecs
            kwargs["decorr_eigenvals"] = eigenvals
            kwargs["decorr_scaling"] = kwargs.get("scaling_factor", 2.5)
...
class HistogramEqualization(StretchAlgorithm):
    ...
    def compute_stretch(image_data: np.ndarray, **kwargs) -> Tuple[float, float, float, float, float, float]:
        ...
        try:
            if image_data.ndim != 3 or image_data.shape[2] < 3:
                # Handle grayscale
                # For histogram equalization, we don't actually need to set min/max
                # as the transform is different, but we'll use approximations
                min_val = np.min(image_data)
                max_val = np.max(image_data)
                return min_val, max_val, min_val, max_val, min_val, max_val
            
            # For RGB, we compute the histogram equalization on each channel
            # and store parameters for the actual transformation
            minR, maxR = np.min(image_data[:,:,0]), np.max(image_data[:,:,0])
            minG, maxG = np.min(image_data[:,:,1]), np.max(image_data[:,:,1])
            minB, maxB = np.min(image_data[:,:,2]), np.max(image_data[:,:,2])
            
            # Store flag for histogram equalization
...
class AdaptiveEqualization(StretchAlgorithm):
    ...
    def compute_stretch(image_data: np.ndarray, **kwargs) -> Tuple[float, float, float, float, float, float]:
        ...
        try:
            ...
            # For RGB, we compute the adaptive equalization parameters
            minR, maxR = np.min(image_data[:,:,0]), np.max(image_data[:,:,0])
            minG, maxG = np.min(image_data[:,:,1]), np.max(image_data[:,:,1])
            minB, maxB = np.min(image_data[:,:,2]), np.max(image_data[:,:,2])
            
            # Store CLAHE parameters for downstream processing
            kwargs["use_clahe"] = True
            kwargs["clahe_clip_limit"] = kwargs.get("clip_limit", 0.01)
            kwargs["clahe_tile_size"] = kwargs.get("tile_size", 8)
...
STRETCH_ALGORITHMS = {
    "min_max": MinMaxStretch,
    "percentile_1": lambda img, **kw: PercentileStretch.compute_stretch(img, low_percentile=1.0, high_percentile=99.0, **kw),
    "percentile_2": lambda img, **kw: PercentileStretch.compute_stretch(img, low_percentile=2.0, high_percentile=98.0, **kw),
    "percentile_5": lambda img, **kw: PercentileStretch.compute_stretch(img, low_percentile=5.0, high_percentile=95.0, **kw),
    "gaussian_2sigma": lambda img, **kw: GaussianStretch.compute_stretch(img, sigma_factor=2.0, **kw),
    "gaussian_3sigma": lambda img, **kw: GaussianStretch.compute_stretch(img, sigma_factor=3.0, **kw),
    "square_root": SquareRootStretch,
    "logarithmic": LogarithmicStretch,
    "decorrelation": DecorrelationStretch,
    "histogram_eq": HistogramEqualization,
    "adaptive_eq": AdaptiveEqualization,
}
...
def compute_stretch(algorithm_id: str, image_data: np.ndarray, **kwargs) -> Tuple[float, float, float, float, float, float]:
...
def apply_stretch_transform(image_data: np.ndarray, algorithm_id: str, stretch_values: Tuple[float, float, float, float, float, float], 
                           **kwargs) -> np.ndarray:
    ...
    if algorithm_id == "decorrelation" and "decorr_eigenvecs" in kwargs:
        try:
            # Unpack decorrelation parameters
            mean_vec = kwargs["decorr_mean"]
            eigenvecs = kwargs["decorr_eigenvecs"]
            eigenvals = kwargs["decorr_eigenvals"]
            scaling = kwargs["decorr_scaling"]
            ...
            scaled_eigenvals = eigenvals * scaling
            pca_data = pca_data * np.sqrt(scaled_eigenvals / eigenvals)
            
            # Transform back to original space
            transformed_data = np.dot(pca_data, eigenvecs.T) + mean_vec
            ...
            logger.error(f"Error applying decorrelation stretch: {e}")
            return image_data
    ...
    # For histogram equalization
        ...
        try:
            ...
            for i in range(c):
                result[:,:,i] = exposure.equalize_hist(image_data[:,:,i])
    ...
    # For adaptive equalization (CLAHE)
        ...
        try:
            ...
            tile_size = kwargs.get("clahe_tile_size", 8)
            
            # Apply CLAHE to each channel
            h, w, c = image_data.shape
            result = np.zeros_like(image_data)
            
            # Calculate tile grid size based on tile_size parameter
